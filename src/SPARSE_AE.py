...
class SPARSE_AE:
    ...
    def __init__(self, model_path, data_path = None, is_training = False,
        ...
        # SPARSE_AE Parameters
        self.model_name     = "SPARSE"
        self.validation_size= 0.20      # 20% of the data will be used for validation
        self.regularization_beta = 1e-5
    ...
    def create_optimizer(self):
        ...
        # Loss
        l2_loss = tf.add_n([tf.nn.l2_loss(v) for v in t_vars if 'bias' not in v.name])
        l2_loss = self.regularization_beta * l2_loss
        self.de_loss = tf.reduce_mean(tf.pow(self.en_input - self.decoded, 2) + l2_loss)
