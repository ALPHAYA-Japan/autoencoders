...
class DENOISING_AE:
    ...
    def __init__(self, model_path, data_path = None, is_training = False,
        ...
        # DENOISING_AE Parameters
        self.model_name     = "DENOISING"
    ...
    def train_initialization(self, hard_load = True, en_iterations = 1, de_iterations = 1):
        ...
        self.labels     = tf.placeholder(tf.float32, shape = self.input_shape, name = 'labels')
    ...
    def decoder(self,x, is_training=True, reuse=False):
        with tf.variable_scope("decoder", reuse=reuse):
            ...
            net = layers.linear(x, output_dim, scope = 'de_fc1',
                                w_initializer = self.w_initializer,
                                b_initializer = self.b_initializer)
    ...
    def create_optimizer(self):
        # Loss
        self.de_loss = tf.reduce_mean(tf.pow(self.labels - self.decoded, 2))
    ...
    def train(self, max_epoches, show_images = False):
        ...
        random_modes = ["poisson","gaussian","s_and_p","speckle"]
        f, a = plt.subplots(4, 10, figsize=(10, 4))

        for epoch in range(max_epoches):
            for i in range(self.num_batches):
                x_batch, _    = self.data.train.next_batch(self.batch_size)
                x_batch       = 1.0 - np.asarray(x_batch)
                random_mode   = np.random.choice(random_modes)
                x_batch_noisy = np.asarray([utils.noisy(random_mode,img) for img in x_batch])
                _, t_loss = self.session.run([self.de_opt, self.de_loss],
                                              feed_dict = {self.labels : x_batch,
                                                           self.en_input: x_batch_noisy})

                if i % 500 == 0:
                    x_batch, _ = self.data.valid.next_batch(self.batch_size)
                    x_batch       = 1.0 - np.asarray(x_batch)
                    random_mode   = np.random.choice(random_modes)
                    x_v_batch_noisy = np.asarray([utils.noisy(random_mode,img) for img in x_batch])
                    v_loss = self.session.run(self.de_loss,
                                              feed_dict = {self.labels : x_batch,
                                                           self.en_input: x_v_batch_noisy})
                    ...
                    if show_images == True:
                        g = self.session.run(self.decoded,
                                             feed_dict = {self.en_input: x_v_batch_noisy})
                        for j in range(10):
                            a[0][j].imshow(x_v_batch_noisy[j])
                            a[1][j].imshow(g[j])
                            a[2][j].imshow(x_v_batch_noisy[10+j])
                            a[3][j].imshow(g[10+j])
                        f.show()
                        plt.draw()
                        plt.pause(0.001)
            ...
            preds = self.session.run(self.decoded,
                                     feed_dict = {self.en_input: x_v_batch_noisy})
            grid = self.construct_image_grid(x_v_batch_noisy, preds, 20, 480, 240)
    ...
    def generate(self, source, destination = None, samples = 20, grid_width=480, grid_height=240):
        ...
        if os.path.isfile(source):
            ...
            batch = 1.0 - batch
            random_mode = np.random.choice(random_modes)
            batch = np.asarray([utils.noisy(random_mode,img) for img in batch])
